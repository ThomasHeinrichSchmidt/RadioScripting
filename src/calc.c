...
double operation(char operator, double pre, double post)
{
   switch (operator) {
      ...
      case '\\': return (double) div(pre, post).quot;
      case '*': return pre * post;
      case '+': return pre + post;
      case '-': return pre - post;
   }
...
int main(int argc, char *argv[])
{
   ...
   while (*str != '\0') {
      ...
      if (isspace(*str)) {
         ...
         str++;
      } else if ((number = strtold(str, &str)) == 0.0L && (number = strtoconst(str, &str)) == 0.0L && strStart == str) {
         /* NaN */                                                  // strtold() interprets a floating-point value in a byte string pointed to by str. 
         insertToken(currentScope, OP, normalise(*str), 0);         // If endptr is not (char **)NULL, strtold(ptr,endptr) will set the pointer pointed 
         str++;                                                     // to by endptr to the first byte of the “final string of unrecognized characters”. 
      } else {                                                      // If no number is recognized, the return value is 0.0L and the complete string remains 
         /* Number or 0.0L */                                       // unrecognized (strStart == str). An actual number 0 will increase str by contrast.
