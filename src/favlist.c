...
int add_to_list(struct strings_list *list, const char *title, const char *url) {
	...
	// parse url like http://b1.mediayou.net/embedded/playURL.php?id=13081&sc=N9XX_AAC for id
	int id = 0; 
	if (strstr(url, "b1.mediayou.net") != NULL) {
		char *c;
		if ((c = strstr(url, "?id=")) != NULL) {
			if (1 == sscanf(c+4, "%i", &id)) id = id;
		}
	}
	item->id1 = id / 256;
	item->id2 = id % 256;
